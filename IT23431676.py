...
def _read_output(is_chat: bool, output_locator) -> str:
    # For textareas the value is set via JS .value, so we must use evaluate()
    # inner_text() / text_content() return empty for JS-populated textarea values
    try:
        ...
        pass
    # Fallback: Playwright input_value()
    try:
        ...
        pass
    # Last resort: inner_text / text_content
    try:
...
def _find_chat_locators(page, timeout_ms: int):
    ...
    while time.time() < deadline:
        ...
        # Strategy 1: Match by placeholder text (most reliable)
        try:
            # Input field contains "English" in placeholder
            input_by_ph = page.locator('textarea[placeholder*="English"]').first
            # Output field contains "Sinhala" in placeholder
            ...
            if (input_by_ph.count() > 0 and output_by_ph.count() > 0
                ...
                print("Debug: Located input/output textareas via placeholder.")
                return input_by_ph, output_by_ph, action
        except Exception:
            pass

        # Strategy 2: Match by placeholder using broader keywords
        try:
            ...
            for ta in all_textareas:
                try:
                    ...
                    if any(kw in ph for kw in ["english", "input", "type", "singlish"]):
                        input_loc = ta
                    elif any(kw in ph for kw in ["sinhala", "output", "result", "transliterat"]):
                        output_loc = ta
                except Exception:
                    pass
            if input_loc and output_loc:
                action = page.get_by_role("button", name=re.compile(r"Transliterate", re.IGNORECASE)).first
                print("Debug: Located input/output textareas via keyword matching.")
                return input_loc, output_loc, action
        except Exception as e:
            last_debug = str(e)

        # Strategy 3: fallback — pick first two visible textareas in DOM order
        try:
            ...
            if len(visible) >= 2:
                action = page.get_by_role("button", name=re.compile(r"Transliterate", re.IGNORECASE)).first
                print(f"Debug: Fallback — using first two of {len(visible)} visible textareas.")
    ...
    # Dump debug info if all strategies failed
    try:
        meta = page.evaluate(
...
def _parse_args():
    ...
    parser.add_argument("--excel", default="IT23191242.xlsx")
    ...
    parser.add_argument("--url", default="https://www.pixelssuite.com/chat-translator")
    ...
    parser.add_argument("--type-delay-ms", type=int, default=10)
    ...
    with sync_playwright() as p:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                # Click Clear button to clear previous input/output completely
                try:
                ...
                # Wait for visible content - retry a few times if empty or unchanged
                actual_output = ""
                ...
                # If output never changed from prev, just use whatever is shown now
        ...
        if args.keep_open and not args.headless:
            try:
                ...
                pass
            print("All tests done. Browser kept open. Close it manually or press CTRL+C.")
            try:
                while True:
                    try:
